    ...
    public partial class FrmPlayer : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (clsUser._AdminEditName != null)
            {
                ...
                string lcresult = _DBMethods.UpdateAccount(_newAccName, _newAccPassword, _newAccScore, _accVersion);
                //if the update was succesfull based on having the same version
                if (lcresult == "1")
                {
                    ...
                    MessageBox.Show("outdated version, please try again");
                this.Close(); }
            }
            else
            {
        ...
        {
            _accVersion = _DBMethods.GetVersion(clsUser._AdminEditName);

            //if an existing account then fill text boxes
            if (clsUser._AdminEditName != null)
            {
